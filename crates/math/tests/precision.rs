//! Floating-point precision and platform-consistency tests for `gravita_math`.
//!
//! These tests are kept as an integration test (not a unit test) because they
//! cover f32 behavior across the whole crate surface rather than any single
//! function — and because the volume would otherwise swamp the lib.rs unit
//! tests for `lerp`/`clamp`/`remap`/`smooth_step`.

use gravita_math::{PI, Vec2, lerp};
...
fn platform_rounding_mode() {
    // Rust's f32::round() uses "round half away from zero".
    assert_eq!(0.5_f32.round(), 1.0, "0.5 rounds to 1 (away from zero)");
    assert_eq!(1.5_f32.round(), 2.0, "1.5 rounds to 2");
    assert_eq!(2.5_f32.round(), 3.0, "2.5 rounds to 3 (away from zero)");
}
